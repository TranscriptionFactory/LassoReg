...
plotResults = function(resultsdf, orig_df, outpath = "") {
  ...
  if (outpath != "") {
    ...
    ggplot2::ggsave(paste0(outpath, "/plot_auc_lasso.png"), plot = plot_auc, width = length(lambdas)*5, height = 6 * ceiling(lambdas/3))

    ggplot2::ggsave(paste0(outpath, "/plot_cfm_lasso.png"), plot = plot_cfm, width = length(lambdas)*5, height = 6 * ceiling(lambdas/3))
  } else {
