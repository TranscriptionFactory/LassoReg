...
plotResults = function(resultsdf, orig_df, outpath = "") {
  ...
  if (outpath != "") {
    ...
    ggplot2::ggsave(paste0(outpath, "/plot_auc_lasso.png"), plot = plot_auc, width = length(lambdas)*4.5, height = 4 * ceiling(length(lambdas/3)))

    ggplot2::ggsave(paste0(outpath, "/plot_cfm_lasso.png"), plot = plot_cfm, width = length(lambdas)*4.5, height = 4 * ceiling(length(lambdas/3)))
  } else {
